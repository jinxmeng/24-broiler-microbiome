...
calcu_PCoA <- function(profile, distance, group, group_colnames = NULL, dim = 2, cumulative_eig = NULL, dis_method = "bray", 
  ...
  if (is.null(cumulative_eig)) {
    ...
    PCoA_points <-  dplyr::select(data.frame(PCoA$points), all_of(1:dim)) %>% rownames_to_column(var = "sample")
  }
  ...
  out = list(dist = distance, points = PCoA_points, dim = dim, eig = PCoA_eig, 
  ...
  if (isTRUE(adonis2)) {
    ...
    group <- group[match(rownames(as.matrix(distance)), group$sample),] %>% as.data.frame(row.names = NULL)
