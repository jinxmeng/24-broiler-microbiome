...
# s3      g2
# s4      g2
diff_test <- function(dat, group, dat_colnames = NULL, group_colnames = NULL,
                      group_order = NULL, center_group = NULL, padj = F,
                      method = "wilcox", paired = F, plab = F) {
  if (!all(c("sample", "val") %in% colnames(dat)) & is.null(dat_colnames)) stop("group field (sample|val)")
  ...
  if (!is.null(center_group)) {
    ...
    if (method == "wilcox") {
      dat_center <- subset(dat, group %in% center_group) %>% select(val) %>% unlist() %>% as.numeric()
      for (i in 1:length(group_order)) {
        dat_i <- subset(dat, group %in% group_order[i]) %>% select(val) %>% unlist() %>% as.numeric()
        wilcox <- wilcox.test(dat_i, dat_center, paired = paired, exact = F)
        group_pair <- paste0(group_order[i], "_vs_", center_group)
        diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(wilcox$p.value), method = "wilcoxon-rank sum")
      }
    } else if (method == "anova") {
      for (i in 1:length(group_order)) {
        group_i <- subset(group, group %in% c(group_order[i], center_group))
        dat_i <- subset(dat, sample %in% group_i$sample)
        anova <- oneway.test(val ~ group, dat_i)
        group_pair <- paste0(group_order[i], "_vs_", center_group)
        diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(anova$p.value), method = "oneway anova")
      }
    } else if (method == "t") {
      for (i in 1:length(group_order)) {
        group_i <- subset(group, group %in% c(group_order[i], center_group))
        dat_i <- subset(dat, sample %in% group_i$sample)
        t <- stats::t.test(val ~ group, dat_i, paired = paired)
        group_pair <- paste0(group_order[i], "_vs_", center_group)
        diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(t$p.value), method = "student's t")
      }
    }
  } else {
    if (method == "wilcox") {
      for (i in 1:(length(group_order) - 1)) {
        for (j in (i + 1):length(group_order)) {
          dat_i <- subset(dat, group %in% group_order[i]) %>% select(val) %>% unlist() %>% as.numeric()
          dat_j <- dat %>% subset(group %in% group_order[j]) %>% select(val) %>% unlist() %>% as.numeric()
          wilcox <- wilcox.test(dat_i, dat_j, paired = paired, exact = F)
          group_pair <- paste0(group_order[i], "_vs_", group_order[j])
          diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(wilcox$p.value), method = "wilcoxon-rank sum")
        }
      }
    } else if (method == "anova") {
      for (i in 1:(length(group_order) - 1)) {
        for (j in (i + 1):length(group_order)) {
          ...
          anova <- oneway.test(val ~ group, dat_ij)
          group_pair <- paste0(group_order[i], "_vs_", group_order[j])
          diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(anova$p.value), method = "oneway anova")
        }        
      } 
      ...
      for (i in 1:(length(group_order) - 1)) {
        for (j in (i + 1):length(group_order)) {
          group_ij <- subset(group, group %in% c(group_order[i], group_order[j]))
          dat_ij <- subset(dat, sample %in% group_ij$sample)
          t <- stats::t.test(val ~ group, dat_ij, paired = paired)
          group_pair <- paste0(group_order[i], "_vs_", group_order[j])
          diff <- tibble::add_row(diff, group_pair = group_pair, pval = as.numeric(t$p.value), method = "student's t")
        }
      }
...
# s3      g2
# s4      g2
diff_test_profile <- function(profile, group, group_order = NULL, group_colnames = NULL, 
  ...
  diff <- purrr::map_dfr(tmp_vec, \(x) diff_test(dat = dplyr::select(dat, sample, val = all_of(x)), 
                                                 group = group, group_order = group_order, method = method, 
                                                 paired = paired, center_group = center_group) %>% 
                   add_column(name = x, .before = "group_pair"))
  if (isTRUE(padj)) diff <- add_column(diff, padj = p.adjust(diff$pval, method = "BH"), .after = "pval")
