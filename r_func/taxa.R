...
plot_compos_manual <- function(otu, group, display = "group", top_n = 12, group_order = NULL, 
  ...
  if(display == "group") {
    ...
    if(is.null(plot_title)) plot_title = "Composition analysis"
    fill_title = paste0(plot_title, " taxa")
    plotdat <- rownames_to_column(dat, var = "name") %>%  gather(., key = "sample", value = "val", -name) %>% 
...
# 输出ggplot对象的list
plot_taxa_boxplot <- function(profle, group, group_order = NULL, group_color = NULL, group_colnames = NULL, method = "wilcox",
                              xlab = "", ylab = "Relative Abundance(%)", legend_title = "group", 
                              show_legend = T, aspect_ratio = 1){
  profle <- data.frame(profle, check.names = F)
  if (!all(c("sample", "group") %in% colnames(group)) & is.null(group_colnames)) stop("group field (sample|group)")
  if (!is.null(group_colnames)) group <- dplyr::rename(data.frame(group, check.names = F), all_of(group_colnames))
  ...
  p_list <- list()
  for (i in name) {
    dat_i <- dat %>% select(sample, group, all_of(i)) %>% rename(index = all_of(i))
      
    source("F:/Code/R_func/diff_test.R")
    comparison <- diff_test(select(dat_i, -group), select(dat_i, -index), group_order = group_order, method = method) %>% 
      filter(pval < 0.05) %>% select(group_pair) %>% unlist() %>% as.character() %>% strsplit(x = ., split = "_vs_")
    
    p <- ggplot(dat_i, aes(x = factor(group, group_order), y = index, color = factor(group, group_order))) + 
